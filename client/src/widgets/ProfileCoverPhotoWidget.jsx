...
const ProfileCoverPhotoWidget = ({ userId }) => {
  ...
  const patchFriend = async () => {
    ...
    if (isFriend) {
      enqueueSnackbar(
        user.firstName + " Removed as your Friend Successfully!!",
        {
          variant: "warning",
      ...
      enqueueSnackbar(user.firstName + " Added as your Friend Successfully!!", {
        variant: "success",
        anchorOrigin: { vertical: "top", horizontal: "right" },
      });
    }
  ...
  return (
    <WidgetWrapper>
      <Box position="relative">
        <Box display="flex" gap="2rem" justifyContent="flex-end">
          ...
          <Box margin="1rem">
            {userId === _id && (
              <Button
                onClick={() => setOpenCoverPhotoPopup(true)}
                variant="contained"
                size="small"
                sx={{
        ...
        <Box
          ...
          marginTop={userId === _id ? "120px" : "155px"}
        >
          ...
          <Stack spacing={1} mt={"3.8rem"} alignItems="center">
            ...
            {(userId===_id)?(friends.length):(user.friends.length)} Friends
            </Typography>
            {userId === _id ? (
              ""
            ) : (
      ...
      <Popup
        ...
        openPopup={openCoverPhotoPopup}
        setOpenPopup={setOpenCoverPhotoPopup}
      >
        <UploadPhoto />
      </Popup>
      <Backdrop
